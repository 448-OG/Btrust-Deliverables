use bitcoin::{
    absolute::LockTime, consensus::encode::deserialize, transaction::Version, Transaction, Txid,
    Weight,
};
use std::str::FromStr;
...
fn main() {
...

    {
        ...
        for tx_input in &decoded_tx.get().input {
            assert_eq!(
                Txid::from_str("49239eb7285257428a2864ae9775b74e1ee9c580d7a24e88bc5d6b760e14cc0c")
                    .unwrap(),
        ...
        let is_segwit =
            if decoded_tx.get().wtxid().as_raw_hash() != decoded_tx.get().txid().as_raw_hash() {
                true
            } else {
                false
            };
